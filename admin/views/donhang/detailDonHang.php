      ...
      <div class="row">
        <div class="card col-12" style="display: flex; flex-direction: row;">
          ...
          <div class="col-4">
            ...
            <div class="card-body">
              <p><strong>Tên Người Đặt:</strong> <?= htmlspecialchars($order['ten_nguoi_nhan']); ?></p>
              <p><strong>Số Điện Thoại:</strong> <?= htmlspecialchars($order['sdt_nguoi_nhan']); ?></p>
              <p><strong>Email:</strong> <?= htmlspecialchars($order['email_nguoi_nhan']); ?></p>
            </div>
          </div>
          <!-- Người Nhận -->
          <div class="col-4">
            ...
            <div class="card-body">
              ...
              <p><strong>Số Điện Thoại:</strong> <?= htmlspecialchars($order['sdt_nguoi_nhan']); ?></p>
              <p><strong>Email:</strong> <?= htmlspecialchars($order['email_nguoi_nhan']); ?></p>
              <p><strong>Địa Chỉ:</strong> <?= htmlspecialchars($order['dia_chi_nguoi_nhan']); ?></p>
              <p><strong>Ghi chú:</strong> <?= htmlspecialchars($order['ghi_chu']); ?></p>
            </div>
          </div>
          <!-- Đơn Hàng -->
          <div class="col-4">
                ...
                <form action="<?= BASE_URL_ADMIN . '/?act=chi-tiet-don-hang&id=' . $order['id'] ?>" method="post">
                  ...
                  <select name="trang_thai_id" class="form-control">
                    <?php foreach ($listTrangThai as $trangThai): ?>
                      <?php
                      // Kiểm tra nếu trạng thái là "Giao hàng thành công" hoặc không hợp lệ, bỏ qua
                      if ($trangThai['id'] <= $order['trang_thai_id'] || $trangThai['id'] == 5) {
                        continue;
                      }
                      ?>
                      <option value="<?= $trangThai['id'] ?>" <?= ($order['trang_thai_id'] == $trangThai['id']) ? 'selected' : ''; ?>>
